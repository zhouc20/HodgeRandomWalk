...
class RRWPLinearNodeEncoder(torch.nn.Module):
    ...
    """
    def __init__(self, emb_dim, out_dim, use_bias=False, batchnorm=False, layernorm=False, pe_name="rrwp", real_emb=True):
        super().__init__()
...
class RRWPLinearEdgeEncoder(torch.nn.Module):
    ...
    def __init__(self, emb_dim, out_dim, batchnorm=False, layernorm=False, use_bias=False,
                 ...
                 overwrite_old_attr=False, real_emb=True):
        super().__init__()
    ...
    def forward(self, batch):
        ...
        if not self.real_emb:
            rrwp_val = torch.zeros(rrwp_val.shape, dtype=torch.float, device=rrwp_val.device)
