    ...
    def __init__(self, dim_in, dim_out):
        ...
        if cfg.posenc_RRWP.enable:
            self.rrwp_abs_encoder = register.node_encoder_dict["rrwp_linear"]\
                (cfg.posenc_RRWP.ksteps, cfg.gnn.dim_inner, real_emb=cfg.posenc_RRWP.real_emb)
            ...
            self.rrwp_rel_encoder = register.edge_encoder_dict["rrwp_linear"] \
                (rel_pe_dim, cfg.gnn.dim_edge,
                 ...
                 fill_value=0., real_emb=cfg.posenc_RRWP.real_emb
                 )

        if cfg.posenc_EdgeRWSE.enable:
            self.EdgeRWSE_encoder = register.edge_encoder_dict["EdgeRWSE_emb"](cfg.posenc_EdgeRWSE.ksteps, cfg.gnn.dim_edge,
                 pad_to_full_graph=cfg.gt.attn.full_attn,
                 add_node_attr_as_self_loop=False,
                 fill_value=0.)

        if cfg.posenc_HodgeLap1PE.enable:
            self.HodgeLap1_encoder = register.edge_encoder_dict["Hodge1Lap_emb"](1, cfg.gnn.dim_edge,
                 pad_to_full_graph=cfg.gt.attn.full_attn,
                 add_node_attr_as_self_loop=False,
                 fill_value=0.)

        if cfg.posenc_RD.enable:
            self.RD_encoder = register.edge_encoder_dict["RD_emb"](1, cfg.gnn.dim_edge,
                 pad_to_full_graph=cfg.gt.attn.full_attn,
                 add_node_attr_as_self_loop=False,
                 fill_value=0.)
